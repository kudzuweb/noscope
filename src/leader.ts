...
/**
 * The reports the IC owes a verdict (R4-2): every `unit.reported` after the last accepted
 * `command.turned`, in log order. A rejected turn answered nothing, so its window's reports
 * carry over to the retry's change report. Command files no report (R4-6), and a report
 * the runtime wrote for a refused unit (R4-7) is one of these like any leader's. The
 * validator's "Reports answered" requires one verdict per report here and none outside.
 */
