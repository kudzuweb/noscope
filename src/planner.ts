...
export const PLANNER_RULES = [
  ...
  "Paths exist: every path input of a deterministic task (a grep's root, a read's path, a git_history's cwd) names a path that exists under the working directory the incident runs in, relative to it or absolute; check_path is exempt, since whether its path exists is its answer. A package in a pnpm workspace lives under the package's own node_modules, not the root's.",
