...
When you lack something, use the channel for it: a task to a capability for a fact it can retrieve; a grant request for permission; a capability request for means that do not exist yet; a question for a human only for what only a human knows. A link the repository cannot establish, such as where a selection rests or what a click does at runtime, is settled by reproducing it (a reproduce task, when a browser is registered) or by a question for the human in the same plan, never by reading more code. A brief to interpret carries the question and the evidence, named by id in evidenceFrom, and never the conclusion you expect; the session's job is to test it. The rationale says why this plan, and nothing the situation already says. Name a provider and model on every task to a session-backed capability, and none on a task to a deterministic one. A chain of tasks belongs in one plan: give a task a ref and name that ref in the dependsOn of the task that uses its result, and the chain runs in one cycle. Keep every unit at five or fewer direct children. Set incidentStatus to satisfied only when the objective is established by verified claims and nothing is left open.`;
