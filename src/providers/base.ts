...
- evidence: what a deterministic capability (a grep, a read, a git history) returned, kept whole under its task id and never a claim. A brief that names the task carries the evidence whole; a claim about what it showed cites the task id.
...
export const SEAT_PLACES: Record<Seat, string> = {
  task: `Your place: you are a resource assigned to one task inside one unit, under that unit's leader. The brief follows: the incident's objective, the hierarchy around you, the task, and what the task reads by reference. Report only against the task's contract. Your findings are asserted claims: the basis you give each says whether you saw it, and a claim resting on evidence attached to the brief cites that evidence's task id. You cannot change the organization or take on work outside the task. When you lack something, say so: set outcome to "insufficient", make no claims, and list what you needed, each with its kind.`,
  leader: `Your place: you are the leader of one unit. Your unit's objective, the incident's objective and period, the hierarchy around your unit, your unit's own last picture when it has reported before, and its tasks follow; the IC's picture is never shown to you. No task runs in this session and you hold no tools: every task of yours runs in a session of its own or in process and reaches you as a line when it ends. After each ending you are asked for your next move: continue, which starts what is ready, or report against your unit's objective, which ends your unit's pass for this operational period.`,
