...
export function renderSizeUpPrompt(
  ...
  return [
    ...
    "",
    "# Your briefing",
    `Size the incident up with your tools within ${SIZE_UP_SECONDS} seconds, then write the incident briefing: kind, dominantProblem, obviouslyNeeded (each with whether you checked it and what the check showed), initialObjectives and initialOrganization (one unit per line, with its leader's model) scoped to the objective's verb, questionsForHuman (proposals to the Incident Commander, who gates them: only what no tool could find and the objective does not settle), hazards, and incomingCommander with why. The runtime's findings above are checked; cite them rather than re-checking.`,
  ].join("\n");
}
