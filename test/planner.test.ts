  ...
  it("renders the incident file as the ten sections in the design's order, the IC's situation last", () => {
    ...
    expect(text).toMatchInlineSnapshot(`
      ...
      warned on, and applied anyway:
        ...
        - Independent work runs together: units and tasks with no dependency between them belong in the same period and start at once, so a unit that can start now goes in this plan, never the next. A task waits only for a task whose result it takes, named in its evidenceFrom.tasks; a dependsOn on a task the dependent does not read is a wait for nothing, and a code reading never waits behind a reproduce it does not need (run 004's code unit sat idle for the 278 seconds of a reproduce it did not read, then read its files one after another). A wait the plan needs for another reason is kept, and the rationale says why.
      rejected last cycle:
