...
│   ├── planner.ts        # the planner's call: renders the eleven sections, records plan.proposed, redrafts once on a correction
...
match, Reports answered, Deterministic only, Drops match), and applied as a plan is: `command.turned`
...
against the root; and by four rules of its own:
