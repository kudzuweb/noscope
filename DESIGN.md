...
one unit run at once. The fifth run (R5-12) measured 0.87x, 0.76x, 0.80x and 0.97x
against 1.00x, 1.40x, 1.00x and 1.15x possible: the plans allowed the parallelism and the
dispatcher's `relatedUnits` took it away, since it serializes two whole units when any
unended task of one depends on any unended task of the other, so a unit's independent
tasks wait behind another unit's pass; and every report said the picture changed, so
each period ran one wave and the run's seats summed to its wall time, 55 minutes for 27
minutes of sessions (`docs/first-incident.md`, "Fifth run"). Holding the dependent task
rather than its unit is the round 6 candidate. The startup cost per session
...
The IC's context is the other clock (R5-11): a resumed session keeps every past
briefing, and through round 4 every briefing carried the whole incident file, which is
why run 004's last turn read a 170k context and sat one turn short of the handoff in an
incident of three periods. Since round 5 a resumed IC call carries the change report and
the file's changed sections rather than the whole file (Step 4), so the IC's context
grows by what happened and not by the file's size times the number of turns. What
happened is most of the file when every period lands a report, claims, tasks and a new
situation: the fifth run's resumed command turns added about 18k, 21k and 28k tokens
each and its reviews 7k to 16k, the IC's context grew 30k to 47k a period, and the
handoff came on the fourth command turn at 128,967 (R5-12). The turns
themselves are not padded: run 004's largest command turn was 8.9k characters, about
2.2k tokens, of decisions against 16.9k output tokens billed, the rest the model's
reasoning before it answered (checked 2026-09-15 23:05), so nothing in round 5 asks a
turn for fewer words.
## Open questions
