...
Still proposed rather than ruled, and settled by building them: {==the storage tables in Step 2==}{>>which schema<<}{id="c66" by="user" at="2026-09-13T03:55:04.866Z"}{>>The SQLite tables in Step 2: incidents, units, tasks, claims, events, grants and their columns. Named plainly now.<<}{id="r67" by="AI" at="2026-09-13T04:02:00.000Z" re="c66"}, {==the command list in Step 7==}{>>?<<}{id="c67" by="user" at="2026-09-13T03:55:25.799Z"}{>>The set of commands under noscope incident in Step 7: create, show, tree, step, run, events, sop, answer, grant. "CLI surface" was jargon for that; named plainly now.<<}{id="r68" by="AI" at="2026-09-13T04:02:00.000Z" re="c67"}, and the first incident. Opus 5 as the planner model and the validator rules as written are accepted for now. Everything else in this document is a ruling.
## Vocabulary
| Term | Meaning |
|---|---|
...
| Resolved 2026-09-12, see Decisions. {==The MCP equipment server, now ruled, still has one untested part: whether a provider's built-in tool filter also filters MCP tools in headless mode. The options that were weighed are under "Custom==}{>>test it<<}{id="c61" by="user" at="2026-09-13T03:25:22.302Z"}{>>hello did we test this and rule on it<<}{id="c63" by="user" at="2026-09-13T03:53:51.566Z" re="c61"}{>>Yes, both. Tested 2026-09-12 22:30 with a stdio MCP server: the --tools filter left the MCP tool callable under no flag, --tools "", --tools "Read" and --tools naming it. Ruled the same hour: custom equipment reaches a session over MCP with --tools "", the decision row beginning "Custom equipment reaches a session over MCP". The spike is spikes/mcp-tools-filter/run.sh. This row stays only because your comment is on it; it is marked resolved.<<}{id="r69" by="AI" at="2026-09-13T04:02:00.000Z" re="c63"} equipment for sessions" in the Reference section. | No. v0 sessions use built-in tools only. | Any session-backed capability that needs function equipment cannot be written until one path is chosen and tested. The leading asserted is the runtime serving its equipment registry as an MCP server, because the session then sees each piece of equipment as a native tool with a schema and nothing to learn. |
