...
tasks by their seconds, and the factor the summed seconds make over it, so the measured
factor is read against what the plan's dependencies allowed: run 004's period 1 would
read 278 seconds of path for 278 seconds of work, 1.00x possible, since the reproduce was
the only session task that ran; the code unit's reading waited on a grep that had failed
at once and never needed the reproduce. The planner's warning "Independent work runs
together" (Step 5) and the IC's review ask (Step 4) are what move the possible factor.
