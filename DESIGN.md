...
| `summarize` | The predicate of a capability's claims that are many and alike (grep's `matches`): the planner sees them in full only in the cycle after they land, then one line per task unless the IC's situation names them in `evidence`; the capability's other predicates, such as a verified absence, stay in full. |
...
2. Claims, each line showing its status and basis and its provenance. A claim with the predicate its capability declares as `summarize` (grep's `matches`, in v0) appears in full only in the cycle after it lands, or when the last situation names it in `evidence`; the rest of its task's claims collapse to one line per task: the inputs, the claim count, and the files with counts.
