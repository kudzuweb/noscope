...
| A resumed call reports usage for that call alone, not the conversation's total. | The same three calls: `num_turns` 2 on each, cache writes growing by one turn each time (8.7k, 9.0k, 9.3k). Cache reads were 0 on every `resume.sh` call (`--tools ""`, schema changed per call) and 10.2k on the `agents.sh` resumed call (`--tools Agent`, schema also changed); the cause is open and R3-3's live test settles it. |
