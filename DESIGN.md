...
| Resolved 2026-09-12, see Decisions. {==The MCP equipment server, now ruled, still has one untested part: whether a provider's built-in tool filter also filters MCP tools in headless mode. The options that were weighed are under "Custom==}{>>test it<<}{id="c61" by="user" at="2026-09-13T03:25:22.302Z"}{>>Tested. The --tools filter leaves MCP tools alone: the echo tool answered under no --tools flag, --tools "", --tools "Read" and --tools naming it, with --strict-mcp-config and an allowedTools entry. --tools "" is the cleanest shape: the session sees only the runtime's equipment, 2.5k tokens of context. Recorded as a decision and a verified fact; the spike is in spikes/mcp-tools-filter/. Row removed from open questions.<<}{id="r65" by="AI" at="2026-09-13T03:35:00.000Z" re="c61"} equipment for sessions" in the Reference section. | No. v0 sessions use built-in tools only. | Any session-backed capability that needs function equipment cannot be written until one path is chosen and tested. The leading asserted is the runtime serving its equipment registry as an MCP server, because the session then sees each piece of equipment as a native tool with a schema and nothing to learn. |
