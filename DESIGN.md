...
runtime reads each IC call's whole input context from the usage it recorded (the
`inputTokens` on `command.turned`, `plan.reviewed` or `command.failed`, whichever was
last, when it names the unit's current session) and, before the command turn and before
