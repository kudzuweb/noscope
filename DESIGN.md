...
| Claude Code built-in tool | Only inside a capability's session, named in that capability's equipment: `Read`, `Grep`, `Glob`, and `Bash` under an allowlist of read-only commands (`READ_ONLY_SESSION_COMMANDS` in `src/equipment/builtin.ts`: the in-process list `ls`, `cat`, `head`, `tail`, `wc`, `find`, `stat`, plus `grep`, `rg`, `diff`, `sort`, `uniq`, `tree`, `pwd`, `echo`, `which`, `basename`, `dirname`, `realpath`, and the read-only git subcommands `git log`, `git status`, `git diff`, `git show`, `git blame`, `git ls-files`, `git rev-parse`, each a whole entry). A capability may instead declare `default` to give its session Claude Code's whole built-in set. |
