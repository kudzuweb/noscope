...
| Inferred links are worked | Every inferred link in the plan's situation names what settles it: a task in this plan by its ref, an open task by its id, a question this plan raises by its position, or a reproduce task. Every claim id the situation names, in `proven`, `inferred` and `keep`, is a claim in the incident (checked under Dependencies resolve). |
