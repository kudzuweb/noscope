...
unit's id and `fromUnitId`, the closed unit's. A reassignment is open while it is recorded,
not dropped and not taken; neither event changes state, and a replay reads the same set.
...
its claims by id, which the planner's section 11 lists while it is open and the next plan
must give to a new unit that names it in `takes`, or which the IC drops by beginning the
instructions with `drop:`, closing it as it is recorded; the taking unit's leader is
oriented with the instructions and the claims (Step 6). A
...
one `report.reviewed` per verdict, one `unit.reassigned` per reassign verdict (R4-4),
units closed, by `closeUnits` and by verdict, a reassigned unit's open tasks cancelled,
...
10. The situation from the last applied plan, as the planner wrote it: what changed, the hypothesis, the observed claims it rests on, the inferred links with what settles each, and the claims to keep in view; "(none)" before the first applied plan. Placed here, with section 11, because the provider caches the unchanged front of a prompt and these sections change every cycle.
11. The reassignments open (R4-4): each with its id, the closed unit and its objective, the cycle and report it was closed on, the IC's instructions and why, and the unit's claims by id; "(none)" until the IC reassigns. Every one is taken by a new unit in this plan naming it in `takes` (the rule "Reassignments taken", Step 5). R4-5 folds this into the situation the IC writes.
...
opens with the orientation: the incident, the period, the situation, the hierarchy and the
