...
one unit doing session work and the code unit's three independent investigates ran inside
its leader's session one after another (`docs/first-incident.md`). Tasks inside a leader's
session still run one at a time, and the startup cost per session stays unless sessions
